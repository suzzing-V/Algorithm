...
public class Main {
    ...
    public static void quick_sort(int[] arr_x, int[] arr_y, int start, int end) {
        ...
        while (left < right) { //left와 right가 뒤바뀌면 멈춤 //pivot에서 뒤바뀔 수 밖에 없음
            while (arr_y[left] < pivot_y || (arr_y[left] == pivot_y && arr_x[left] < pivot)) //조건에 안 맞을 때까지 left 이동하기
                left++;
            while (arr_y[right] > pivot_y || (arr_y[right] == pivot_y && arr_x[right] > pivot)) //조건에 안 맞을 때까지 right 이동하기
                right--;
