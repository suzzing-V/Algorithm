...
class Game:
    ...
    def start_game(self):
        ...
        self.my_player = self.players[int(input()) - 1].name
        ...
        i = 1
        while i <= 30: 
            self.deck.append(random.randrange(1, 14))
            i += 1
    ...
    def play_round(self):
        ...
        l = len(self.deck)
        while(i <= 4):
            print(f"{self.players[i - 1].name} (현재 점수: {self.players[i - 1].score})")
            players_card.append(self.deck[l - i])
            print(f"뽑은 카드: {self.deck[l - i]}\n")
            i += 1
        ...
        max_score = max(players_card)
        min_score = min(players_card)
        get_score = max_score - min_score
        if(get_score != 0):
            i = 0
            while i < 4:
                if(players_card[i] == max_score):
                    self.players[i].score += get_score
                    self.players[i].round_wins += 1
                    print(f"{self.players[i].name}님이 {get_score} 점을 얻었습니다 \^_^/")
                i += 1
            print()
    ...
    def game_result(self):
        ...
        i = 0
        while(i < 4):
            if self.players[i].name == self.my_player:
                print(f"{i + 1}등 - *{self.players[i].name}* : {self.players[i].score}점")
            else:
                print(f"{i + 1}등 - {self.players[i].name} : {self.players[i].score}점")
            i += 1
        ...
        print("     게임 순위 - 승리 횟수")
        print("=============================")
        self.players.sort(key = lambda x: (-x.round_wins, x.name))
        i = 0
        while(i < 4):
            if self.players[i].name == self.my_player:
                print(f"{i + 1}등 - *{self.players[i].name}* : {self.players[i].round_wins}번")
            else:
                print(f"{i + 1}등 - {self.players[i].name} : {self.players[i].round_wins}번")
            i += 1
